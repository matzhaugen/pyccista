...
curl -L https://sourceforge.net/projects/swig/files/swig/swig-4.2.1/swig-4.2.1.tar.gz/download?use_mirror=kumisystems# -o tarfiles/swig-4.2.1.tar.gz  &&\
tar -xzvf tarfiles/swig-4.2.1.tar.gz > /dev/null 2>&1 &&\
# cd swig-3.0.12 && ./configure > /dev/null 2>&1 && make -j4 > /dev/null 2>&1 && make install -j4 > /dev/null 2>&1 && cd ..
cd swig-4.2.1 && ./configure --without-pcre --prefix=$HOME/.local && make -j4 && make install -j4 && cd ..
